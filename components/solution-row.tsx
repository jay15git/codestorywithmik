...
function getListGridClass(
  difficulty: boolean,
  topics: boolean,
  companies: boolean
) {
  const mobile = difficulty
    ? "grid-cols-[4.75rem_minmax(0,1fr)_2.5rem_3rem]"
    : "grid-cols-[minmax(0,1fr)_2.5rem_3rem]"
  ...
  if (difficulty) {
    return cn(mobile, "md:grid-cols-[4.75rem_minmax(0,1fr)_2.5rem_3rem]")
  }
  if (topics) {
    return cn(
      mobile,
...
export function SolutionRow({ solution, variant = "grid" }: SolutionRowProps) {
  const { visibleColumns } = useSolutionViewContext()
  const showDifficulty = visibleColumns.includes("difficulty")
  const showTopics = visibleColumns.includes("topics")
  const showCompanies = visibleColumns.includes("companies")
  ...
  if (variant === "list") {
    return (
      <div
        className={cn(
          "group/row duration-quick relative px-(--spacing-solution-row-x) py-(--spacing-solution-row-y) transition-colors ease-smooth-out hover:bg-muted/40",
          LIST_ROW_GRID,
          getListGridClass(showDifficulty, showTopics, showCompanies)
        )}
      >
        <Link
          href={solutionHref}
        ...
        {showDifficulty ? (
          <div className="pointer-events-none relative z-10 flex min-h-7 items-center">
            <DifficultyBadge difficulty={solution.difficulty} size="xs" plain />
          </div>
        ) : null}

        <div className="pointer-events-none relative z-10 flex min-h-7 min-w-0 items-center">
          <TitleUnderline className="block max-w-full truncate text-sm leading-snug font-medium">
            {solution.title}
          </TitleUnderline>
        </div>
        ...
        <div className="pointer-events-none relative z-10 flex min-h-7 items-center justify-center">
    ...
    <article className="group/row relative flex min-h-32 cursor-pointer flex-col rounded-lg border bg-card p-(--spacing-solution-card) md:min-h-36">
      ...
      <div className="pointer-events-none relative z-10 flex min-w-0 flex-1 flex-col">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 text-base leading-snug font-medium text-balance md:text-lg">
        ...
        {(topicItems.length > 0 ||
          ...
          <div className="flex flex-1 flex-col">
            {topicItems.length > 0 && (
              <div className="pointer-events-auto mt-(--spacing-solution-title-meta)">
                <CompactTagOverflow items={topicItems} />
              </div>
            )}

            {visibleCompanies.length > 0 && (
              <div className="pointer-events-auto mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
